...
In \texttt{target/jars} there are 4 \texttt{.raj} files, one for each Bean, along with the script used to build the \texttt{.jar} files, and the script to rename the $\texttt{.jar}\rightarrow\texttt{.raj}$ and \textit{viceversa}.
